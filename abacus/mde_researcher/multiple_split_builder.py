...
class MultipleSplitBuilder:
    ...
    def _build_splits_grid(self):
        return list(
            BaseSplitElement(el[0], el[1])
            for el in itertools.product(
                self.group_sizes, np.arange(1, self.iterations_number + 1)
            )
        )

    def _update_strat_params(self):
        """Update stratification columns, because of columns names duplicated problem."""
        self.stratification_params.cols = [
            el + "_strat"
            if el
            not in [
                self.stratification_params.main_strata_col,
                self.stratification_params.split_metric_col,
            ]
            else el
            for el in self.stratification_params.cols
        ]
        self.stratification_params.cat_cols = [
            el + "_strat" for el in self.stratification_params.cat_cols
        ]
    ...
    def calc_injected_metrics(self, guests_for_injects: pd.DataFrame) -> pd.DataFrame:
        ...
        for pair in matched:
            guests_for_injects_copy[f"{pair[0]}_{pair[1]}"] = (
                guests_for_injects_copy[f"{pair[0]}"] * pair[1]
            )
    ...
    def _build_split(
        ...
        return guests_groups[
            [
                self.stratification_params.id_col,
    ...
    def collect(self) -> pd.DataFrame:
        ...
        """
        guests_data_with_strata = self.split_builder._assign_strata(
            self.split_builder.split_data
        )
        ...
        for split in self.split_grid:
            ...
            guests_split = self._build_split(
                guests_data_with_strata,
                split.control_group_size,
                split.target_group_size,
                split.split_number,
            )
            experiment_guests = experiment_guests.merge(
                guests_split[[self.stratification_params.id_col, experiment_column]],
                on=self.stratification_params.id_col,
                how="left",
            )
            del guests_split
        guests_data_with_strata = guests_data_with_strata.merge(
            experiment_guests, on=self.stratification_params.id_col, how="inner"
        )
